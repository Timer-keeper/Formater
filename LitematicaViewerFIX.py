...
def ConAly():
    threading.Thread(
        target=lambda: subprocess.run(
            ["python", grs(os.path.join("script", "LitContainer.py"))]
        ),
        daemon=True,
    ).start()
...
def insert_table(block_state, count, simple_type):
    ...
    count_table.insert(
        ...
        "end",
        image=img,
        values=(block_id_display, str(count), convert_units(count)),
    )
    litem.update_idletasks()
...
if __name__ == "__main__":
    ...
    menu_AnaSet.add_checkbutton(
        ...
        variable=DoLifr,
        command=lambda: hide(frame_spawn, DoLifr),
        font=(DefaultFont, 10),
    )
    menu_AnaSet.add_checkbutton(
    ...
    btn_import = CTkButton(
        frame_top, text="Import导入", command=import_file, font=(DefaultFont, 10)
    )
    ...
    hide(
        frame_middle,
        DoAnal,
        lambda: frame_middle.pack(side=tk.LEFT, fill=tk.BOTH, expand=True),
    )
    # - 中容器顶部
    ...
    label_times = tk.Label(
        frame_Times, text="Times倍数", font=("microsoft yahei ui", 16, "bold")
    )
    ...
    hide(
        frame_data,
        DoStat,
        lambda: frame_data.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True),
    )
