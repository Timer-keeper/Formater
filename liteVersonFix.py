...
def litVerFix(version: int) -> None:
    ...
    new_file_name = (
        f"{file_name_without_extension}_v{nbt_file.tag['Version']}{file_extension}"
    )
...
if __name__ == "__main__":
    ...
    for idx, option in enumerate(ver_opt):
        tk.Radiobutton(root, text=option, variable=selected_version, value=idx).pack(
            anchor=tk.W
        )
    ...
    def on_button_click():
        version = (
            7
            if selected_version.get() == 0
            else 5 if selected_version.get() == 1 else 4
        )
