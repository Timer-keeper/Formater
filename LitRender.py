        ...
        for y, _ in enumerate(blocks[x]):
            for z, _ in enumerate(blocks[x][y]):
                if not blocks[x][y][z] or blocks[x][y][z] == "minecraft:tripwire":
                    ...
                    if (
                        isinstance(blocks[x + 1][y][z], str)
                        and isinstance(blocks[x - 1][y][z], str)
                        and fu
                        and isinstance(blocks[x][y][z + 1], str)
                        and isinstance(blocks[x][y][z - 1], str)
                    ):
                        blocks[x][y][z] = None
                        continue
                ...
                mode = (
                    1
                    if "slab" in str(blocks[x][y][z])
                    else (
                        2
                        if "carpet" in str(blocks[x][y][z])
                        else 3 if "pane" in str(blocks[x][y][z]) else 0
                    )
                )
                ...
                try:
                    color = CCrgb(
                        grs(
                            os.path.join(
                                "block", f"{blocks[x][y][z].split(':')[1]}.png"
                            )
                        )
                    )
                    ...
                    if color == (0.0, 0.0, 0.0):
                        color = CCrgb(
                            grs(
                                os.path.join(
                                    "item", f"{blocks[x][y][z].split(':')[1]}.png"
                                )
                            )
                        )
                        mode = 2
...
def init_opengl(display):
    ...
    glClearColor(
        int(color_map["MC"][1:3], 16) / 255.0,
        int(color_map["MC"][3:5], 16) / 255.0,
        int(color_map["MC"][5:7], 16) / 255.0,
        1.0,
    )  # 设置背景颜色
...
class OpenGLView(OpenGLFrame):
    def __init__(self, parent, blocks, rotate=True, **kwargs):
        ...
        self.pos_blocks = [
            [[None] * (zl + 1) for _ in range(yl + 1)] for _ in range(xl + 1)
        ]
