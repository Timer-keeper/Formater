    ...
    for x in range(width):
        for y in range(height):
            for z in range(length):
                ...
                is_on_face = (
                    (faces[0] and z < wall_thickness)  # 前面
                    or (faces[1] and z >= width - wall_thickness)  # 后面
                    or (faces[2] and x < wall_thickness)  # 左面
                    or (faces[3] and x >= length - wall_thickness)  # 右面
                    or (faces[4] and y < wall_thickness)  # 下面
                    or (faces[5] and y >= height - wall_thickness)  # 上面
                )
                if hollow:
                    if is_on_face:
...
def change_Schematic(schematic, change_list, limit: tuple, file_name):
    ...
    print(
        f"BoundsRegion: xmin={minx}, xmax={maxx}, ymin={miny}, ymax={maxy}, zmin={minz}, zmax={maxz}"
    )
    # 动态设置边界条件
    ...
    print(
        f"BoundsInputFixed: xmin={xmin}, xmax={xmax}, ymin={ymin}, ymax={ymax}, zmin={zmin}, zmax={zmax}"
    )
        ...
        for x, y, z in block_positions:
            if xmin <= x <= xmax and ymin <= y <= ymax and zmin <= z <= zmax:
                ...
                if block.id in replace_dict:
                    ...
                    if properties:
                        new_block = litemapy.BlockState(
                            replace_dict[block.id], **properties
                        )
                    else:
